...
地图事实观察与用户接管是独立通道。视图、图层及其级联变化只更新 MapContext，不能根据变化类型或发生时间推断操作者。地图手势和应用内用户命令入口明确发出接管信号；异步命令必须在开始执行前发出信号。运行中的指令始终绑定原 scope；已完成指令后的新输入可以捕获替换后的地图，同时保留会话历史。

输入草稿在指令通过前置检查并进入消息历史后才被消费。附件展示属于草稿，文件引用属于可用数据，二者分别管理生命周期。运行状态由统一规则决定是否接受输入，解释执行失败仍属于运行阶段。

GIS 策略遵循显式意图和最小动作：导入不隐含定位、改样式或高亮；歧义通过普通回复澄清。桥接层实时传递正文和阶段进度，工具调用只在前端执行请求形成后提交。会话租约由服务端校验、到期释放；浏览器取消是提前释放途径，不能替代服务端回收。
